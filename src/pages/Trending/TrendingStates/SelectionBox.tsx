...
import { selectOption } from '../TrendingStates/HomeVote';
interface Props {
  listoption: {
    id?: number;
    name?: string;
...
export const SelectionBox = (props: Props) => {
  const { listoption, selected, setSelected, indexCriteria } = props;
  const [isActive, setIsActive] = useState<boolean>(false);

  const handleChoseOption = (option: any, indexCriteria: number): void => {
    setSelected((prev) => [...prev, (selected[indexCriteria] = option)]);
    setIsActive(false);
  };
    ...
    <div className="mt-[10px] w-[100%] h-[40px] rounded-[8px]">
      ...
      >
        <div className="flex items-center min-h-[40px]">
          {selected && !selected[indexCriteria] ? (
            <p>Select an item</p>
          ) : (
            <>
            ...
            {listoption &&
              listoption?.map((option: any) => (
                <div
                  ...
                  key={option?.id}
                  onClick={() => handleChoseOption(option, indexCriteria)}
                >
